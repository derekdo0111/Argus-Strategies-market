...
## 维护规则（AI Agent 和开发者必读）

> 修改龟龟策略任何计算公式时，必须遵守以下规则。违反任一条 = 回归 Bug。
...
|---|------|------|
| ① 计算模块 Python 代码 | 实际执行 |
| ② `rules/v2/*.yaml` | 规则权威定义 |
| ③ `turtle-coordinator.md`（本文档） | SPEC 流程文档 |
| ④ `tests/` | 回归测试 |
...
- **硬门**（screener / CQ / PR）：不通过即淘汰 (硬排除) 或标记 (软门)，不走后续流程
...
- 阈值变更 → 同步更新测试边界值
- 两层测试：
  - **SPEC 合规** (`test_spec_compliance.py`)：验证代码有没有按本文档做
  - **单元测试** (`test_*.py`)：验证按本文档做的代码算得对不对
